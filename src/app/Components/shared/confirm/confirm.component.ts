  ...
  template: ` <div>
      ...
      <div *ngIf="showComponent">
        ...
        >
          <div class="d-flex flex-column justify-content-center">
            <h4 style=" font-weight: bold; margin: 2px; ">
              Please enter the one-time password <br />
              to verify your account
            </h4>
            <span class="my-2"
              >A code has been sent to {{ form.value.email }}
              <p *ngIf="remainingTime === 0" class="text-danger my-auto">
                Otp Expired
              </p></span
            >
            <div>
              <ng-otp-input
                ...
                [config]="{ length: 6 }"
              ></ng-otp-input>
              <div *ngIf="showError && remainingTime !== 0">
                <p class="text-danger">The Entered Otp is not valid</p>
              </div>
            </div>
            <div class="my-2 d-flex justify-content-between">
              ...
              <div
                *ngIf="showComponent && remainingTime === 0"
                class="d-flex gap-2"
              >
                <button
                  pButton
    ...
    <div *ngIf="!showComponent">
      <button
        ...
        type="button"
        class="btn btn-primary"
        (click)="toggleComponentVisibility(1)"
      >
        Send Otp
      </button>
    </div>`,
      ...
      next: (response: any) => {
        if (response.success) {
          ...
          if (response.data.type == 2) {
           this.confirmService.setemail(response.data.email)
           this.router.navigate(['forgotpasssword'])
          }
          this.confirmService.getdata(response.data.type);
        } else {
          this.showError = true;
        }
      },
